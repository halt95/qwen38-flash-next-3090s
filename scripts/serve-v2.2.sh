...
#
# Topology selection (opt-in in this release): AUTO_TOPO unset or 0 (the default) never runs the selector and keeps
# the fixed settings the release was qualified with (GPUs 0,1,2,3 in bus order, P2P check skipped, NCCL_P2P_LEVEL=SYS,
# NCCL_PROTO=LL): the environment and the vllm command are exactly those of the script without the selector.
# AUTO_TOPO=1 has this script call scripts/topo_select.py to pick the actual GPU order and P2P/NCCL settings from live
# topology instead of assuming the reference host's 0,1,2,3 (meant for NVLink-bridged pairs, hosts with more than 4
# GPUs and multi-socket hosts), falling back to the fixed settings on any doubt; AUTO_TOPO=strict runs it too, but
# refuses to start (exit 1) rather than guess when more than 4 GPUs are visible and CUDA_VISIBLE_DEVICES is not set,
# and also when the selector cannot run at all (fails, times out, or hands back an unusable file). Any other AUTO_TOPO
# value prints a note and leaves the selector off. The knobs below matter only with the selector on. AUTO_TOPO_NCCL=default
# lets a host with no P2P at all drop NCCL_PROTO=LL instead of forcing it (legacy, the default, has not been compared with NCCL's choice in a
# no-P2P configuration -- see below); AUTO_TOPO_BIND=1 asks for NUMA/CPU binding when the chosen lane's four GPUs
# share one node (do not also pass vLLM's --numa-bind). LANE_GPUS=auto opts into picking four idle GPUs out of more
# than four visible; LANE_CPUS further restricts the binding cpulist; TOPO_TIMEOUT (1-600 s, default 20) bounds the
# selector. The selector's diagnostics land under $CACHE_ROOT/topology (the same cache root as VLLM_CACHE_ROOT below).
# docs/topology-selector.md has the full decision rules.
#
...
# --- topo_select integration (opt-in in this release; docs/topology-selector.md) -------------------------------------
# The selector runs only with AUTO_TOPO=1 or AUTO_TOPO=strict. Unset, empty or 0 (the default) takes the legacy path
# below and nothing else: no caller snapshot, no TOPO_* variable exported, no launch prefix, so the environment and the
# exec line equal those of the script without the selector. Any other value: a note, then the same legacy path.
case "${AUTO_TOPO:-0}" in
  0) topo_on=0 ;;
  1 | strict) topo_on=1 ;;
  *) echo "topo: note: AUTO_TOPO='$AUTO_TOPO' is not 0, 1 or strict; the selector stays off"; topo_on=0 ;;
esac
# The exec prefix lives in this shell variable and is set only from a validated selector decision, so a
# TOPO_LAUNCH_PREFIX inherited from the caller is never used (with the selector off it reaches vllm untouched, as it
# would without the selector).
topo_launch_prefix=""

# today's behaviour, byte for byte -- the only path when the selector is off, and the fallback whenever the selector
# can't be trusted (missing, times out, errors, or hands back something unparseable). With "fallback" as $1 (selector
# on only) it first restores the caller's CUDA_VISIBLE_DEVICES from the snapshot (so nothing a failed selector run
# left behind survives), treats a whitespace-only value as unset, as the selector itself does, and clears the
# selector's two outputs and the launch prefix.
legacy_topo_env() {
  if [ "${1:-}" = fallback ]; then
    if [ "$TOPO_CALLER_SET_CUDA_VISIBLE_DEVICES" = 1 ]; then
      CUDA_VISIBLE_DEVICES="$TOPO_CALLER_CUDA_VISIBLE_DEVICES"
    else
      unset CUDA_VISIBLE_DEVICES
    fi
    case "${CUDA_VISIBLE_DEVICES:-}" in *[![:space:]]*) ;; *) unset CUDA_VISIBLE_DEVICES ;; esac
    export TOPO_LAUNCH_PREFIX="" TOPO_DECISION_JSON=""
    topo_launch_prefix=""
  fi
  ...
  # Without P2P: VLLM_SKIP_P2P_CHECK=1 changes nothing on its own -- it is vLLM's own default once no P2P is found --
  # and NCCL_PROTO=LL also governs the no-P2P tensor-parallel path, where it has not been compared with NCCL's own protocol choice. Neither
  # setting is "harmless" so much as "not known to hurt"; the selector below chooses them from the topology NVML reports.
  export VLLM_SKIP_P2P_CHECK=1 NCCL_P2P_LEVEL=SYS NCCL_PROTO=LL
}

# The env file the selector writes must be exactly its 8-line format -- the header, then these 7 statements in this
# order, each a plain `export NAME=<shell-quoted value>` (or `unset NAME` for the two NCCL variables) -- before it is
# sourced. Anything else (a truncated file, a missing statement, an extra line) means "do not source it".
topo_env_file_ok() {
  local f="$1" i=0 line
  local val="([A-Za-z0-9_@%+=:,./-]+|'[^']*'(\"'\"'[^']*')*)"
  local -a want=(
    "^# topo_select v1 status=(ok|legacy) reason=[A-Za-z0-9_.-]+$"
    "^export CUDA_VISIBLE_DEVICES=$val$"
    "^export CUDA_DEVICE_ORDER=$val$"
    "^export VLLM_SKIP_P2P_CHECK=$val$"
    "^(export NCCL_P2P_LEVEL=$val|unset NCCL_P2P_LEVEL)$"
    "^(export NCCL_PROTO=$val|unset NCCL_PROTO)$"
    "^export TOPO_LAUNCH_PREFIX=('numactl --physcpubind=[0-9,-]+ --preferred=[0-9]+ --'|'')$"
    "^export TOPO_DECISION_JSON=$val$"
  )
  [ -f "$f" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
    [ "$i" -lt "${#want[@]}" ] || return 1
    [[ $line =~ ${want[$i]} ]] || return 1
    i=$((i + 1))
  done < "$f" || return 1
  [ "$i" -eq "${#want[@]}" ]
}

# The selector could not be used: legacy settings, or exit 1 under AUTO_TOPO=strict (strict means strict).
topo_unavailable() {
  if [ "$AUTO_TOPO" = strict ]; then
    echo "topo: $1; AUTO_TOPO=strict refuses to start without a selector decision (AUTO_TOPO=1 falls back to the legacy settings)"
    exit 1
  fi
  echo "topo: $1, using legacy settings"
  legacy_topo_env fallback
}

if [ "$topo_on" = 0 ]; then
  legacy_topo_env
else
  # Snapshot caller intent *before* any default (ours or the selector's) can overwrite it: an unset variable is not
  # the same thing as one the caller happened to set to what the default would have picked anyway, and
  # topo_select.py needs to tell the two apart. Only these six are part of the topology decision. Taken only when the
  # selector runs, so the selector-off environment carries no TOPO_CALLER_* variables.
  for v in CUDA_VISIBLE_DEVICES CUDA_DEVICE_ORDER VLLM_SKIP_P2P_CHECK NCCL_P2P_LEVEL NCCL_PROTO PLE_HOME; do
    if [ "${!v+set}" = set ]; then
      export "TOPO_CALLER_SET_$v=1" "TOPO_CALLER_$v=${!v}"
    else
      export "TOPO_CALLER_SET_$v=0"
    fi
  done
  export TOPO_LAUNCH_PREFIX="" TOPO_DECISION_JSON=""
  # TOPO_TIMEOUT: a whole number of seconds, 1-600; anything else gets the default (never raw shell arithmetic on it).
  # The selector gets the same validated value, and the outer watchdog allows 5 s more, then KILLs 5 s after that.
  topo_timeout=20
  case "${TOPO_TIMEOUT:-}" in
    '') ;;
    [1-9] | [1-9][0-9] | [1-9][0-9][0-9])
      if [ "$TOPO_TIMEOUT" -le 600 ]; then topo_timeout="$TOPO_TIMEOUT"; else
        echo "topo: note: TOPO_TIMEOUT=$TOPO_TIMEOUT is above 600 s; using 20"; fi ;;
    *) echo "topo: note: TOPO_TIMEOUT='$TOPO_TIMEOUT' is not a whole number of seconds from 1 to 600; using 20" ;;
  esac
  # diagnostics beside the vLLM cache: the same root this script exports as VLLM_CACHE_ROOT further down
  topo_json_dir="${CACHE_ROOT:-$HERE/.vllm-cache-v2.2}/topology"
  tmp_topo_env=""
  if tmp_topo_env="$(mktemp 2>/dev/null)" && [ -n "$tmp_topo_env" ]; then
    trap 'rm -f -- "$tmp_topo_env" 2>/dev/null || :' EXIT
    rc=0
    TOPO_TIMEOUT="$topo_timeout" timeout --kill-after=5 "$((topo_timeout + 5))" \
      "$VENV/bin/python" "$HERE/scripts/topo_select.py" --env-out "$tmp_topo_env" --json-dir "$topo_json_dir" || rc=$?
    if [ "$rc" = 3 ]; then
      # strict mode refused to guess; the selector already put its reason on stderr.
      exit 1
    elif [ "$rc" != 0 ]; then
      topo_unavailable "selector unavailable (rc=$rc)"
    elif ! topo_env_file_ok "$tmp_topo_env"; then
      topo_unavailable "selector unavailable (rc=0, unusable env file)"
    else
      # shellcheck disable=SC1090  -- generated by topo_select.py and validated line by line above
      if ! source "$tmp_topo_env"; then
        topo_unavailable "could not apply the selector's env file"
      else
        topo_launch_prefix="$TOPO_LAUNCH_PREFIX"
      fi
    fi
    rm -f -- "$tmp_topo_env" 2>/dev/null || :
    trap - EXIT
  else
    topo_unavailable "could not create a temporary file for the selector"
  fi
fi

if [ "${SERVE_PRINT_ENV_AND_EXIT:-0}" = 1 ]; then
  # Test hook only. Everything the topology decision touches (CUDA_VISIBLE_DEVICES, CUDA_DEVICE_ORDER,
  # VLLM_SKIP_P2P_CHECK, NCCL_P2P_LEVEL, NCCL_PROTO, the launch prefix, TOPO_DECISION_JSON) is final the moment the
  # block above exits -- nothing later in the script touches any of them -- so this is the earliest point a test (or a
  # dry run by hand) can observe the real decision without needing a GPU, a driver, or a real vllm install. Prints one
  # "name=value" line per variable, then the exec-time launch prefix already word-split the way the final exec will
  # split it, then exits 0 before anything below (which assumes real hardware) runs.
  echo "SERVE_PRINT_ENV_AND_EXIT:"
  echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-}"
  echo "CUDA_DEVICE_ORDER=${CUDA_DEVICE_ORDER:-}"
  echo "VLLM_SKIP_P2P_CHECK=${VLLM_SKIP_P2P_CHECK:-}"
  echo "NCCL_P2P_LEVEL=${NCCL_P2P_LEVEL-<unset>}"
  echo "NCCL_PROTO=${NCCL_PROTO-<unset>}"
  echo "TOPO_LAUNCH_PREFIX=$topo_launch_prefix"
  echo "TOPO_DECISION_JSON=${TOPO_DECISION_JSON:-}"
  prefix_argv=()
  if [ -n "$topo_launch_prefix" ]; then read -r -a prefix_argv <<< "$topo_launch_prefix"; fi
  printf 'EXEC_ARGV_PREFIX:'
  if ((${#prefix_argv[@]})); then printf ' [%s]' "${prefix_argv[@]}"; fi
  printf '\n'
  exit 0
fi
# --- end topo_select integration -------------------------------------------------------------------------------
...
# topo_launch_prefix is deliberately unquoted: it word-splits into a numactl invocation (or nothing) taken from the
# selector's env file, built there from a cpulist and a couple of integers, never from anything a caller or a
# checkpoint path could inject (the env file is checked against exactly that shape before it is sourced). It is always
# empty with the selector off. Do not pass vLLM's --numa-bind when a prefix is in use.
exec ${topo_launch_prefix:+$topo_launch_prefix} "$VENV/bin/vllm" serve "$CKPT" \
